...
var getHeaderRow = function (headers) {
...
function XMLStream (/* no need for options */) {
...
XMLStream.prototype._transform = function (doc, encoding, callback) {
  var self = this
  // push header first and only 1
  if (this.options === null) {
    // auto detect map
    console.log(Object.keys(doc))
    this.map = Object.keys(doc).reduce((a, b) => {
      a[b] = helpers.getType(doc[b])
      return a
    }, {})
    // auto detect headers
    this.headers = Object.keys(doc).map(a => {
      return {width: doc[a].length, caption: a}
    })
    this.headersLength = Object.keys(doc).length

    // push header up front
    this.push(sheetFront)
    this.push(getHeaderColumns(this.headers))
    this.push('<x:sheetData>')
    this.push(getHeaderRow(this.headers))
  }
